...
let Router = {
  ...
  showScene(Scene, params, ...options) {
    ...
    return (...args) => {
      const next = args.pop()
      if (params) {
        const promises = _.map(params, (param) => (_.isFunction(param) ? param(...args) : param) )

        Promise.all(promises)
          .then((p) => {
            Scene.params(...p)
            if (Router.hypeDocument.currentSceneName() !== Scene.name) // new versions of Hype are not performing this check
